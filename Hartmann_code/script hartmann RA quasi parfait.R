...
vars_binaires <- cols_to_include_in_table1[df_HartmannCoelio %>%
                                             select(all_of(cols_to_include_in_table1)) %>%
                                             summarise(across(everything(), ~ all(. %in% c(0, 1, NA)))) %>%
                                             unlist()]

# Remplacement des NA par 0 pour ces variables uniquement
df_temp <- df_HartmannCoelio %>%
  mutate(across(all_of(vars_binaires), ~ tidyr::replace_na(., 0)))

# Génération du tableau
table1unadjusted <- df_temp %>%
  tbl_summary(
    include = all_of(cols_to_include_in_table1),
    missing = "no",  # les NA ont été remplacés
